<template>
  <div>
    <h2>{{ pageName }}</h2>
    <div class="Card" v-if="merchant_token != ''">
      <h3>Hi! {{merchant.merchantName}}</h3>
    ...
    <h3 v-else @click="toLogin()">请登录商家账户</h3>
  </div>
</template>
...
export default {
  ...
  methods: {
    changePassword() {
      ...
      } else if (this.password == this.merchant.password) {
        console.log("输入的新密码不能与老密码相同");
      } else {
        console.log("新密码为：" + this.password);
        this.$emit("updateMerchantPassword", this.password);
      }
    },
    toLogin() {
