  ...
  <div>
    <el-row class="demo-autocomplete">
      ...
      <el-col :span="12">
        ...
        <!-- :trigger-on-focus="false"设置输入后匹配 -->
      </el-col>
    </el-row>
...
export default {
  data() {
    return {
      restaurants: [],  //建议列表的所有内容，在loadAll
      state1: '',
      state2: '',
      state: ''
    };
  ...
  methods: {
    querySearch(queryString, cb) {
      ...
      // 调用 callback 返回建议列表的数据（cb是回调函数call-back的缩写）
      //queryString是输入内容；result为返回的建议内容
      //有输入内容，则建议内容与输入内容匹配；若无输入内容，则显示所有建议列表的内容
      ...
      //远程搜索→等待一个随机数？？？→会出一个等待的样式

    },
    createFilter(queryString) {   //用过滤器匹配输入内容与列表内容
      return (restaurant) => {
        return (restaurant.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0);
        //从头开始（indexof匹配位置），小写匹配则选中
      };
    },
    loadAll() {
      return [
